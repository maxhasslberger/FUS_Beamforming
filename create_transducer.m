function [karray_t, mask_active, mask2el_delayFiles] = create_transducer(kgrid, t_name, sparsity_name, t_pos, t_rot, active_tr_ids)
...
[elementAll_pos, mask2el_ids] = el2mask_indexing(elementAll_pos_orig, n_arr_elements);

% Obtain transducer element ids in the right order -> t_mask
mask_active = mask2el_ids(:, active_tr_ids);
mask_active= sort(mask_active(:)); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
elementAll_pos = elementAll_pos(:, mask_active);
...
function [elementAll_pos, mask2el_ids] = el2mask_indexing(elementAll_pos_orig, n_arr_elements)
